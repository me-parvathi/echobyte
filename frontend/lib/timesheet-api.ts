...
export class TimesheetApiService {
  ...
  static async getMyTimesheetBatch(
    ...
    return api.get<TimesheetBatchResponse>(
      `/timesheets/my/batch?${params.toString()}`
    );
  }
  ...
  static async createMyDailyEntry(
  ...
  static async createMyWeeklyTimesheet(
    weeklyData: WeeklyTimesheetFormData
  ): Promise<Timesheet> {
    return api.post<Timesheet>("/timesheets/my/weekly", weeklyData);
  }

  // Update timesheet
  static async updateMyTimesheet(
    timesheetId: number,
    updateData: Partial<Timesheet>
  ): Promise<Timesheet> {
    return api.put<Timesheet>(
      `/timesheets/my/timesheet/${timesheetId}`,
      updateData
    );
  }

  // Submit timesheet
  static async submitMyTimesheet(
    timesheetId: number
  ): Promise<TimesheetSubmissionResponse> {
    return api.post<TimesheetSubmissionResponse>(
      `/timesheets/my/timesheet/${timesheetId}/submit`
    );
  }

  // Bulk save daily entries
  static async bulkSaveDailyEntries(
    entries: DailyEntryFormData[]
    ...
    results.forEach((result, index) => {
      if (result.status === "fulfilled") {
        ...
        errors.push(
          `Failed to save entry for ${entries[index].WorkDate}: ${result.reason}`
        );
      }
    });
  ...
  static async getTimesheetStatus(
    timesheetId: number
  ): Promise<Timesheet["StatusCode"]> {
    const timesheet = await api.get<Timesheet>(
      `/timesheets/my/timesheet/${timesheetId}`
    );
